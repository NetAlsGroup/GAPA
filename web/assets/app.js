...
const DISABLED_NODES_KEY = "gapa_disabled_nodes";
function getDisabledNodes() {
  try {
    return JSON.parse(localStorage.getItem(DISABLED_NODES_KEY) || "[]");
  } catch (e) {
    return [];
  }
}
function setNodeDisabled(id, disabled) {
  let list = getDisabledNodes();
  if (disabled) {
    if (!list.includes(id)) list.push(id);
  } else {
    list = list.filter((x) => x !== id);
  }
  localStorage.setItem(DISABLED_NODES_KEY, JSON.stringify(list));
}
function isNodeDisabled(id) {
  return getDisabledNodes().includes(id);
}
...
function updateServerSelect() {
  ...
  const activeNodes = sources.filter((s) => !isNodeDisabled(s.id));
  serverSelect.innerHTML = activeNodes.map((s) => `<option value="${s.id}">${s.label}</option>`).join("");
  serverSelect.value = activeNodes.some((s) => s.id === current) ? current : (activeNodes[0]?.id || "local");
  updateRunGpuOptions();
  updateEvalSummary();
  renderConfigServers();
}
...
function renderConfigServers() {
  const listEl = document.getElementById("config-server-list");
  if (!listEl) return;
  if (!sources.length) {
    listEl.innerHTML = `<tr><td colspan="5" style="padding:40px;text-align:center;color:var(--muted);">暂无配置的计算节点。</td></tr>`;
    return;
  }
  listEl.innerHTML = sources
    .map((s, idx) => {
      const isLocal = s.id === "local";
      const isDisabled = isNodeDisabled(s.id);
      const type = s.meta?.type || "remote";
      const typeLabel = isLocal ? "本地节点" : (type === "local" ? "本地节点" : "远程节点");
      const delBtn = isLocal ? "" : `<button class="btn" style="padding:4px 8px; color:#ef4444; border:none; background:transparent; font-size:12px;" onclick="deleteNode('${s.id}')">删除</button>`;

      const toggleHtml = isLocal ? "" : `
        <label class="switch">
          <input type="checkbox" ${isDisabled ? "" : "checked"} onchange="toggleNodeStatus('${s.id}', this.checked)">
      ...
      return `
        <tr style="border-bottom: 1px solid var(--panel-border);">
          ...
          <td style="padding: 12px 16px; font-weight: 600;">${s.label}</td>
          <td style="padding: 12px 16px; font-family: monospace; color: var(--text-dim);">${s.base || "127.0.0.1"}</td>
          <td style="padding: 12px 16px;">
            <div style="display: flex; align-items: center; gap: 12px;">
              <span style="font-size: 11px; padding: 2px 8px; border-radius: 4px; background: ${isLocal ? "#eff6ff" : "#f8fafc"}; color: ${isLocal ? "#2563eb" : "#64748b"}; border: 1px solid ${isLocal ? "#dbeafe" : "#e2e8f0"};">
                ${typeLabel.toUpperCase()}
              </span>
              ${toggleHtml}
            </div>
          </td>
          <td style="padding: 12px 16px; text-align: right;">${delBtn}</td>
        </tr>
      `;
    })
    .join("");
...
window.toggleNodeStatus = (id, enabled) => {
  setNodeDisabled(id, !enabled);
  updateServerSelect();
  updateServerMultiList();
  render();
  updateLockScopeOptions();
  log(`节点 ${id} 已${enabled ? "启用" : "禁用"}`);
};

window.deleteNode = (id) => {
  const idx = sources.findIndex((s) => s.id === id);
  if (idx > 0) { // Can't delete local
    if (!confirm(`确定要移除节点 ${id} 吗？`)) return;
...
function updateLockScopeOptions() {
  if (!lockScope) return;
  const activeNodes = sources.filter((s) => !isNodeDisabled(s.id));
  const opts = [{ id: "all", label: "所有服务器" }, ...activeNodes.map((s) => ({ id: s.id, label: s.label }))];
...
function renderLockStatus(data) {
  ...
  if (!entries.length) {
    lockStatusBox.innerHTML = `<div style="font-size:13px;color:var(--muted);padding:12px;text-align:center;">暂无活动锁定任务</div>`;
    return;
  }
  lockStatusBox.innerHTML = entries
    .filter(([sid]) => !isNodeDisabled(sid))
    .map(([sid, info]) => {
      if (info?.error) {
        return `
          <div class="stat" style="border-color:#fecaca; background:#fff1f2;">
            <div class="stat-label" style="color:#b91c1c;">${sid} (ERROR)</div>
            <div class="stat-value" style="font-size:13px; color:#991b1b;">${info.error}</div>
          </div>`;
      }
      const active = info?.active;
      const statusLabel = active ? "Running" : "Idle";
      const statusColor = active ? "#10b981" : "#94a3b8";
      ...
      return `
        <div class="stat">
          ...
          <div class="stat-value" style="font-size: 15px;">Device Index: ${devices}</div>
          <div style="font-size:11px;color:var(--muted);margin-top:8px; line-height:1.4;">
            Backend: ${info?.backend || "-"}<br>
            Expires: ${exp}<br>
            Reserved Memory: ${info?.mem_mb != null ? info.mem_mb + " MB" : "-"}
            ${info?.note === "no_available_gpu" ? " <span style='color:#ef4444;'>· No Available GPU</span>" : ""}
          </div>
        </div>
      `;
    })
...
function updateRunGpuOptions() {
  ...
  if (runGpuMulti) {
    runGpuMulti.innerHTML = options
      .map((o) => {
        return `
          <label class="server-multi-label" style="border: 1px solid var(--panel-border); background: #ffffff; padding: 8px 12px; border-radius: 8px; cursor: pointer; transition: all 0.2s;">
            <input type="checkbox" class="run-gpu-check" value="${o.id}" style="margin-right: 10px;" />
            <div style="flex: 1;">
              <div style="font-weight: 600; font-size: 13px;">${o.name}</div>
              <div style="font-size: 11px; color: var(--muted);">Index #${o.id}</div>
            </div>
          </label>
        `;
      })
...
function render() {
  cardsDiv.innerHTML = sources
    ...
    .map((src) => {
      ...
      const statusColor = data.error ? "#ef4444" : "#10b981";
      const statusText = data.error ? "Offline" : "Online";

      const gpuHtml = gpus.length
        ? gpus
          .map(
            (g) => `
              <div class="gpu-item" style="border: 1px solid var(--panel-border); background: #f8fafc; padding: 12px; border-radius: 8px; margin-top: 8px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                  <div style="font-weight: 700; font-size: 13px; color: var(--text-ink);">GPU ${g.id} · ${g.name}</div>
                  <div style="font-size: 11px; font-weight: 600; color: #3b82f6;">${pct(g.gpu_util_percent)} Core</div>
                </div>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                  <div style="font-size:11px; color:var(--muted);">
                    Memory: <span style="color:var(--text-ink); font-weight:500;">${fmt(g.used_mb)} / ${fmt(g.total_mb)} MB</span>
                  </div>
                  <div style="font-size:11px; color:var(--muted);">
                    Power: <span style="color:var(--text-ink); font-weight:500;">${fmt(g.power_w, " W")}</span>
                  </div>
                </div>
              </div>`
          )
          .join("")
        : `<div style="font-size:12px;color:var(--muted);margin-top:12px; padding: 12px; background: #f8fafc; border-radius: 8px; border: 1px dashed var(--panel-border);">No GPU devices detected</div>`;

      return `
        <div class="card ${expanded ? "card--expanded" : ""}">
          <div class="card-inner" onclick="toggle('${src.id}')" style="cursor: pointer;">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
              <div>
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                  <span style="width: 8px; height: 8px; border-radius: 50%; background: ${statusColor};"></span>
                  <div class="card-name" style="margin: 0; font-size: 16px;">${title}</div>
                </div>
                <div style="font-size: 12px; color: var(--muted); font-weight: 500;">
                  ${src.meta?.type === "local" ? "LOCAL AGENT" : "REMOTE SERVER"} · ${data.hostname || "N/A"}
                </div>
              </div>
              <div style="font-size: 11px; font-weight: 600; color: var(--muted); background: #f1f5f9; padding: 4px 8px; border-radius: 4px;">
                ${data.time || "--"}
              </div>
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; margin-bottom: 12px;">
              <div class="stat" style="padding: 12px; background: #f8fafc; border: none; min-height: auto;">
                <div class="stat-label">CPU Usage</div>
                <div class="stat-value" style="font-size: 18px;">${pct(cpu.usage_percent)}</div>
              </div>
              <div class="stat" style="padding: 12px; background: #f8fafc; border: none; min-height: auto;">
                <div class="stat-label">Memory</div>
                <div class="stat-value" style="font-size: 18px;">${pct(mem.percent)}</div>
              </div>
              <div class="stat" style="padding: 12px; background: #f8fafc; border: none; min-height: auto;">
                <div class="stat-label">GPUs</div>
                <div class="stat-value" style="font-size: 18px;">${gpus.length}</div>
              </div>
            </div>
            
            ${data.error ? `<div style="color: #ef4444; font-size: 12px; margin-top: 12px; padding: 10px; background: #fef2f2; border-radius: 6px;">Error: ${data.error}</div>` : ""}
          </div>

          <div class="card-expand" style="padding: ${expanded ? "0 20px 20px" : "0"}; max-height: ${expanded ? "1000px" : "0"}; overflow: hidden; transition: all 0.3s ease-out;">
            <div style="border-top: 1px solid var(--panel-border); padding-top: 16px; margin-top: 0;">
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                <div style="font-size: 12px; color: var(--muted);">CPU Load (1m): <span style="color: var(--text-ink); font-weight: 600;">${fmt(cpu.load_avg?.[0], "", "N/A")}</span></div>
                <div style="font-size: 12px; color: var(--muted);">RAM: <span style="color: var(--text-ink); font-weight: 600;">${fmt(mem.used_mb)} / ${fmt(mem.total_mb)} MB</span></div>
              </div>
              ${gpuHtml}
            </div>
          </div>
...
document.getElementById("btn-analyze").addEventListener("click", async () => {
  ...
  try {
    ...
    function renderComparison(compare, selectedPlan) {
      ...
      const barsHtml = rows
        .map((r) => {
          const color = r.isSelected ? "#2563eb" : "#94a3b8";
          const border = r.isSelected ? "1px solid #3b82f6" : "1px solid #e2e8f0";
          const bg = r.isSelected ? "#eff6ff" : "#f8fafc";
          let hint = r.isSelected ? "Active Strategy" : "";
          if (!r.isSelected && baseMs) {
            const rel = r.ms / baseMs;
            hint = rel <= 1 ? `faster ${(baseMs / r.ms).toFixed(1)}x` : `slower ${(rel).toFixed(1)}x`;
          }
          return `
            <div style="display:grid;gap:8px;padding:12px;border-radius:10px;background:${bg};border:${border};">
              <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
                <div style="font-weight:700; font-size:13px;">${r.label}</div>
                <div style="font-size:12px;color:var(--muted);">${r.ms.toFixed(2)} ms ${hint ? `· <span style="color:#2563eb; font-weight:600;">${hint}</span>` : ""}</div>
              </div>
      ...
      gaPanel.innerHTML = `
        <div style="width:100%;display:grid;gap:16px;">
          <div style="display:flex;align-items:center;justify-content:space-between;">
            <div style="font-weight:700; font-size:15px;">Strategy Candidates Comparison</div>
            <div style="font-size:12px;color:var(--muted);">Selected: <b>${selectedLabel}</b></div>
          </div>
    ...
    function renderWarmupSummary(title, data) {
      ...
      if (perIter.length >= 2) {
        ...
        const points = perIter
          ...
          .join(" ");
        spark = `
          <div style="border:1px solid var(--panel-border); border-radius:8px; background:#f8fafc; padding:12px; margin-top:12px;">
            <div style="font-size:11px; color:var(--muted); margin-bottom:8px; font-weight:600; text-transform:uppercase; letter-spacing:0.05em;">Iteration Performance Trend</div>
      ...
      const stateText = state === "completed" ? "Success" : state;

      gaPanel.insertAdjacentHTML(
        "beforeend",
        `
          <div style="display:grid; gap:16px; padding:20px; border-radius:12px; border:1px solid var(--panel-border); background:#ffffff; margin-top:16px;">
            <div style="display:flex; align-items:center; justify-content:space-between;">
              <div style="font-weight:700; font-size:15px; color:var(--text-ink);">Warmup Benchmark Result (${title})</div>
              <div style="font-size:11px; font-weight:700; color:#10b981; background:#ecfdf5; padding:4px 8px; border-radius:4px;">${stateText.toUpperCase()}</div>
            </div>
            
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(140px, 1fr)); gap:16px; padding-bottom:12px; border-bottom:1px solid #f1f5f9;">
              <div>
                <div style="font-size:11px; color:var(--muted); margin-bottom:2px;">Mode</div>
                <div style="font-size:13px; font-weight:600; color:var(--text-ink);">${mode}</div>
              </div>
              <div>
                <div style="font-size:11px; color:var(--muted); margin-bottom:2px;">Avg Iteration</div>
                <div style="font-size:13px; font-weight:600; color:var(--blue-600);">${iterMs ? Number(iterMs).toFixed(2) + " ms" : "-"}</div>
              </div>
              <div>
                <div style="font-size:11px; color:var(--muted); margin-bottom:2px;">Throughput</div>
                <div style="font-size:13px; font-weight:600; color:var(--text-ink);">${throughput ? Number(throughput).toFixed(2) + " ind/s" : "-"}</div>
              </div>
              <div>
                <div style="font-size:11px; color:var(--muted); margin-bottom:2px;">Communication</div>
                <div style="font-size:13px; font-weight:600; color:var(--text-ink);">${commAvg}</div>
              </div>
            </div>

            ${remote ? `
              <div style="font-size:12px; color:var(--muted);">
                <b>Cluster Nodes:</b> <span style="background:#f1f5f9; padding:2px 6px; border-radius:4px;">${remote}</span>
              </div>
            ` : ""}
...
async function historyClear() {
  try {
    ...
    const ids = historyCache.map(it => it.id).filter(Boolean);
    if (ids.length) {
      const resp = await fetch("/api/v1/history", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
...
historySelectAll?.addEventListener("change", (el) => {
  const checked = el.target.checked;
  if (checked) {
    historyCache.forEach(it => historySelected.add(it.id));
  } else {
...
function historyRender() {
  ...
  if (!items.length) {
    historyList.innerHTML = `<div style="color:var(--muted);font-size:13px;padding:20px;text-align:center;">暂无历史记录。</div>`;
          ...
          <div class="history-item-row">
            ...
            <div class="history-item-content">
              <div class="history-item-header">
                <div class="history-item-title">${it.algorithm}</div>
                <div class="history-item-date">${it.timestamp}</div>
              </div>
              <div class="history-item-meta">
                <div style="display: flex; gap: 12px; margin-bottom: 2px;">
                  <span>节点: <span style="color:var(--text-ink);">${it.server_label || it.server_id}</span></span>
                  <span>状态: <span style="color: #10b981; font-weight: 600;">${it.state}</span></span>
                </div>
                <div style="display: flex; gap: 12px;">
                  <span>适应度: <span class="history-score">${score}</span></span>
                  <span>通讯: <span style="color:var(--text-ink);">${commMs}</span></span>
                </div>
              </div>
              <div class="history-actions" style="border-top: none; padding-top: 8px; margin-top: 8px;">
                 <button class="btn btn-secondary" style="padding: 2px 8px; font-size: 11px;" onclick="resumeFromHistory('${it.id}')">继续迭代</button>
              </div>
            </div>
          </div>
        </div>
...
function renderRunCharts(result) {
  ...
  const title = primaryName
    ...
    : "指标曲线";
  runChart.innerHTML = `
    <div style="width:100%; display:grid; gap:20px; padding: 12px;">
      <div>
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom: 12px;">
          <div style="font-weight:700; font-size:14px; color:var(--text-ink);">${title}</div>
          <div style="font-size:11px; color:var(--muted);">
            <span style="display:inline-block; width:8px; height:8px; background:#2563eb; border-radius:2px; margin-right:4px;"></span>${primaryName || "primary"}
            ${secondaryName ? `<span style="display:inline-block; width:8px; height:8px; background:#a855f7; border-radius:2px; margin:0 4px 0 12px;"></span>${secondaryName}` : ""}
          </div>
        </div>
        <div style="border:1px solid var(--panel-border); border-radius:8px; background:#f8fafc; padding:12px;">
          <svg viewBox="0 0 ${w} ${h}" width="100%" height="120" preserveAspectRatio="none">
            ${convPts ? `<polyline fill="none" stroke="#2563eb" stroke-width="2.5" points="${convPts}" />` : ""}
            ${conv2Pts ? `<polyline fill="none" stroke="#a855f7" stroke-width="2.5" points="${conv2Pts}" />` : ""}
            <line x1="${pad}" y1="${h - pad}" x2="${w - pad}" y2="${h - pad}" stroke="#e2e8f0" stroke-width="1.5"/>
          </svg>
        </div>
      </div>
      ...
      <div>
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom: 12px;">
          <div style="font-weight:700; font-size:14px; color:var(--text-ink);">Resource Utilization (Per Round)</div>
          <div style="font-size:11px; color:var(--muted);">
            <span style="display:inline-block; width:8px; height:8px; background:#f59e0b; border-radius:2px; margin-right:4px;"></span>CPU
            <span style="display:inline-block; width:8px; height:8px; background:#16a34a; border-radius:2px; margin:0 4px 0 12px;"></span>MEM
          </div>
        </div>
        <div style="border:1px solid var(--panel-border); border-radius:8px; background:#f8fafc; padding:12px;">
          <svg viewBox="0 0 ${w} ${h}" width="100%" height="120" preserveAspectRatio="none">
            ${cpuPts ? `<polyline fill="none" stroke="#f59e0b" stroke-width="2.5" points="${cpuPts}" />` : ""}
            ${memPts ? `<polyline fill="none" stroke="#16a34a" stroke-width="2.5" points="${memPts}" />` : ""}
            <line x1="${pad}" y1="${h - pad}" x2="${w - pad}" y2="${h - pad}" stroke="#e2e8f0" stroke-width="1.5"/>
          </svg>
        </div>
      </div>
...
window.resumeFromHistory = (id) => {
  ...
  if (confirm(`是否针对 ${it.algorithm} (${it.dataset}) 继续迭代？`)) {
    appVm.setView('algo');
    if (runAlgo) runAlgo.value = it.algorithm;
    if (runDataset) runDataset.value = it.dataset;
    RUN_SERVER_ID = it.server_id || "local";
    if (runServer) runServer.value = RUN_SERVER_ID;
    log(`已加载历史状态 ${id}，准备继续迭代...`);
    startRun({ resume_id: id });
  }
};

async function startRun(opts = {}) {
  const resume_id = opts.resume_id || null;
  ...
  if (runBest) runBest.textContent = "-";
...
async function pollRunStatus(server_id) {
  ...
  if (st.result) {
    ...
    if (runBest && best != null) runBest.textContent = String(best);
    if (runComm) {
...
function initPageAnimations() {
  // Add glow-effect class to interactive cards
  ...
  // Add entrance animation observer for dynamically loaded content
  ...
  console.log('[GAPA] Page animations initialized');
}

// Enhanced progress bar update function
window.updateProgressWithAnimation = function (progressEl, percent) {
  if (!progressEl) return;

  const fill = progressEl.querySelector('.progress-fill') || progressEl;
  fill.style.width = `${Math.min(100, Math.max(0, percent))}%`;

  // Add pulse effect when progress changes
  fill.classList.add('progress-pulse');
  setTimeout(() => fill.classList.remove('progress-pulse'), 500);
};
    ...
    mutation.addedNodes.forEach(node => {
      if (node.nodeType === 1) { // Element node
        if (node.classList?.contains('card')) {
          node.classList.add('glow-effect');
        }
